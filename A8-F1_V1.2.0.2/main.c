...
static int securityCallBackFuncTion( int securityEvent,const	void * data , int dataLen )
{
	...
	switch(securityEvent)
	{
		...
		case UI_ALARM_SENDTOMANAGE:{
			...
			T_Room bajRoom;
			T_Room managerRoom;
		    int bajnum = getBAJNumner();
			getManagerRoom(&managerRoom);
			ret  = getManagerRoom(&managerRoom);
			CHECK_RET(ret<0, "fail to getManagerRoom", );
			ret  =  userSingleServerSendDataToDestRoom(managerRoom,WARN_CMD,(const   unsigned char *)data,sizeof(event_record_t));
			CHECK_RET(ret<0, "fail to talkTranslateCmdToDestWaitAck", );
			for(i = 1;i< bajnum+1; i++){
				ret  = getBAJRoom(&bajRoom,i);
				CHECK_RET(ret<0, "fail to getManagerRoom", continue);
				ret  =  userSingleServerSendDataToDestRoom(bajRoom,WARN_CMD,(const   unsigned char *)data,sizeof(event_record_t));
				CHECK_RET(ret<0, "fail to talkTranslateCmdToDestWaitAck", continue);
				usleep(100*1000);
			}
