...
static int udpSend(S_NetDataPackage  sendPack);
...
int talkCommandInit(T_Room room)
{
	...
	udpSendBuf = createBufferServer(1024*4);
...
int talkTranslateCmdToDestWaitAck(pUdpOps sendServer,T_Room destRoom , unsigned char cmd,const   unsigned char * pData, int len )
{
	...
	if( 0 == getRoomIpAddr(destRoom,&dataPack.remoteInfo.sin_addr.s_addr)){
		...
		dataPack.remoteInfo.sin_port = COMM_UNI_PORT;//
		dataPack.sendServer = sendServer;
		getUtilsOps()->printData(dataPack.dataBody.buf,dataPack.dataBody.len);
		udpSend(dataPack);
...
int callBAJer(int devno)
{
	T_Room  bajRoom;
	int ret;
	ret  = getBAJRoom(&bajRoom,devno);
	CHECK_RET(ret<0, "fail to getManagerRoom", goto fail0 );
	
	if(getLocalTalkState(stateMachinePack)  == TALK_FREE ||getLocalTalkState(stateMachinePack) == CALL_WAIT_ACK )
	{
...
static void *  callThreadFunc(void* arg)
{
	...
	sleep(3);
	//
	if(stateMachinePack->tackState == CALL_WAIT_ACK){
		T_Room dest;
		memset(&dest,0,sizeof(T_Room));
		
		setTalkState(stateMachinePack,TALK_FREE);
		upCmdtoUiFunction(dest,UI_PEER_OFF_LINE,NULL,0);
	}
	
}

int callAllBAJer(void)
{
	pthread_t  thread;
	if (pthread_create(&thread,NULL,callThreadFunc,NULL ) != 0) {
		return -1;
	}
	return 0;
}
